...
export default function AudioPlayer({ audioSrc, title, isDarkMode }: AudioPlayerProps) {
  ...
  return (
    <div className={`mb-4 sm:mb-6 max-w-xs sm:max-w-sm md:max-w-md mx-auto ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
      <div className={`${isDarkMode ? 'bg-gray-800/50 border-gray-600/30' : 'bg-white/80 border-gray-200/50'} p-2 sm:p-3 md:p-4 rounded-xl border backdrop-blur-sm`}>
        {/* Title */}
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3 mb-2 md:mb-3">
          ...
          >
            {isCurrentAudio && isPlaying ? (
              ...
              <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 ml-0.5" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="flex items-center space-x-1 md:space-x-2">
            ...
            <input
              ...
              className={`w-6 sm:w-8 md:w-12 h-1 md:h-1.5 rounded-full appearance-none cursor-pointer ${
                isDarkMode ? 'bg-gray-600/30' : 'bg-gray-200/50'
              }`}
              style={{
